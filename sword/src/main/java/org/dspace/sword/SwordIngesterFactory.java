...
public class SwordIngesterFactory
{
            ...
            throws DSpaceSwordException, SwordError
    {
        ...
        // RLR TODO - this is broken
        ingester = (SwordContentIngester)ConfigurationManager.getInstance("swordv2-server", "content.ingester." + deposit.getMimeType());
        if (ingester != null)
        {
            return ingester;
        }
        ...
        String pkging = deposit.getPackaging();
        String pkgingKey = pkging.substring(pkging.lastIndexOf("/") + 1);
        ingester = (SwordContentIngester)ConfigurationManager.getInstance("swordv2-server", "content.ingester." + pkgingKey);
        if (ingester == null)
        {
    ...
    public static SwordEntryIngester getEntryInstance(Context context, Deposit deposit, DSpaceObject dso)
            throws DSpaceSwordException, SwordError
    {
		SwordEntryIngester ingester = (SwordEntryIngester)ConfigurationManager.getInstance("swordv2-server", "entry.ingester");
		if (ingester == null)
		{
