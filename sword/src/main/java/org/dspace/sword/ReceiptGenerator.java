...
public class ReceiptGenerator
{
			...
			throws DSpaceSwordException, SwordError, SwordServerException
	{
		...
		try
		{
			...
			for (Bundle bundle : result.getItem().getBundles("LICENSE"))
			{
				for (Bitstream bss : bundle.getBitstreams())
				{
					String url = urlManager.getBitstreamUrl(bss);
	...
	{
		List<MDValue> dcv = result.getItem().getMetadata("dc.subject.*");
		for (MDValue val : dcv)
		{
			receipt.getWrappedEntry().addCategory(UriRegistry.DC_NAMESPACE, val.getValue(), val.getValue());
		}
	}

	protected void addCategories(Item item, DepositReceipt receipt)
	{
		List<MDValue> dcv = item.getMetadata("dc.subject.*");
		for (MDValue val : dcv)
		{
			receipt.getWrappedEntry().addCategory(UriRegistry.DC_NAMESPACE, val.getValue(), val.getValue());
		}
	...
	{
		List<MDValue> dcv = result.getItem().getMetadata("dc.date.issued");
		if (dcv.size() == 1)
        {
            try
            {
	...
	{
		List<MDValue> dcv = item.getMetadata("dc.date.issued");
		if (dcv.size() == 1)
        {
            try
            {
	...
	protected void addLastUpdatedDate(DepositResult result, DepositReceipt receipt)
	{
		...
		List<MDValue> dcv = result.getItem().getMetadata(config);
		if (dcv.size() == 1)
        {
            try
            {
	...
	{
		String config = ConfigurationManager.getProperty("swordv2-server", "updated.field");
		List<MDValue> dcv = item.getMetadata(config);
		if (dcv.size() == 1)
        {
            try
            {
