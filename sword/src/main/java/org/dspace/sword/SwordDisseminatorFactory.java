...
public class SwordDisseminatorFactory
{
        ...
        try
        {
            ...
            if (accept != null)
            {
                for (Float q : accept.keySet())
                {
                    for (String format : accept.get(q))
                    {
						...
						format = format.replace("=", "_"); // clean up the string for the plugin manager
                        // RLR FIXME - revisit
                        disseminator = (SwordContentDisseminator)ConfigurationManager.getInstance("swordv2-server", "content.disseminator." + format);
            ...
            if (disseminator == null)
            {
                if (acceptPackaging != null)
                {
					...
					acceptPackaging = acceptPackaging.replace("=", "_"); // clean up the string for the plugin manager
                    // RLR FIXME
                    disseminator = (SwordContentDisseminator)ConfigurationManager.getInstance("swordv2-server", "content.disseminator." + acceptPackaging);
                    if (disseminator != null)
                    {
                        if (accept != null)
                        {
		...
		if (accept != null)
		{
			for (Float q : accept.keySet())
			{
				for (String format : accept.get(q))
				{
					...
					format = format.replace("=", "_"); // clean up the string for the plugin manager
                    // RLR FIXME
					disseminator = (SwordStatementDisseminator)ConfigurationManager.getInstance("swordv2-server", "statement.disseminator." + format);
					if (disseminator != null)
					{
						break;
					}
    ...
    public static SwordEntryDisseminator getEntryInstance()
            throws DSpaceSwordException, SwordError
    {
        SwordEntryDisseminator disseminator = (SwordEntryDisseminator)ConfigurationManager.getInstance("swordv2-server", "entry.disseminator");
        if (disseminator == null)
        {
