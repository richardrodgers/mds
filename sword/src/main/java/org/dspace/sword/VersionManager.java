...
public class VersionManager
{
			...
			throws SQLException, AuthorizeException, IOException
	{
		...
		for (Bundle b : item.getBundles(name))
		{
			...
			throws SQLException, AuthorizeException, IOException
	{
        ...
        bitstream.setMetadataValue("dsl.description", newDesc);
        bitstream.update();
	}
			...
			throws SQLException, AuthorizeException, IOException
	{
		...
		List<Bundle> swords = item.getBundles(swordBundle);
		Bundle archive = null;
		if (swords.size() == 0)
		{
			...
			throws SQLException
	{
		...
		if (item.getBundles(nName).size() == 0)
		{
			return nName;
		}
		else
