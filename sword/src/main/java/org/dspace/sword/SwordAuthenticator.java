...
public class SwordAuthenticator
{
		...
		throws DSpaceSwordException
	{
		...
		try
		{
			...
			BoundedIterator<Community> commIter = Community.findAllTop(context);
			while (commIter.hasNext())
			{
				...
				if (!authAllowed)
				{
					authAllowed = AuthorizeManager.authorizeActionBoolean(swordContext.getAuthenticatorContext(), comm, Constants.READ);
				}
				...
				if (!oboAllowed)
				{
					oboAllowed = AuthorizeManager.authorizeActionBoolean(swordContext.getOnBehalfOfContext(), comm, Constants.READ);
				}
		...
		throws DSpaceSwordException
	{
		...
		try
		{
			...
			List<org.dspace.content.Collection> cols = org.dspace.content.Collection.findAuthorized(authContext, community, Constants.ADD);
			...
			for (Collection coll : cols)
			{
				...
				if (!oboAllowed)
				{
					oboAllowed = AuthorizeManager.authorizeActionBoolean(swordContext.getOnBehalfOfContext(), coll, Constants.ADD);
				}
			...
			while (ii.hasNext())
			{
				...
				if (!authAllowed)
				{
                    ...
                    else
                    {
                        ...
                        {
                            add = AuthorizeManager.authorizeActionBoolean(swordContext.getAuthenticatorContext(), bundles.get(i), Constants.ADD);
                            if (!add)
                            {
                                break;
                            }
            ...
            throws DSpaceSwordException
    {
        ...
        try
        {
            ...
            else
            {
                for (int i = 0; i < bundles.size(); i++)
                {
                    add = AuthorizeManager.authorizeActionBoolean(allowContext, bundles.get(i), Constants.ADD);
                    if (!add)
                    {
                        break;
                    }
